...
const NumberAnalysis645 = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const response = await fetch('/data/power645.jsonl');
        if (!response.ok) {
  ...
  const analyzeData = (data) => {
          ...
          for (let k = j + 1; k < numbers.length; k++) {
            for (let l = k + 1; l < numbers.length; l++) {
              for (let m = l + 1; m < numbers.length; m++) {
                ...
                fiveComponationCount[key] = (fiveComponationCount[key] || 0) + 1;
              }
            }
          }
        }
    ...
    const sortedCombinations = Object.entries(fiveComponationCount)
  ...
  const getNumberColor = (number) => {
    if (number <= 9) return 'primary';
    if (number <= 18) return 'secondary';
    if (number <= 27) return 'success';
    if (number <= 36) return 'warning';
    return 'info';
  };
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Typography variant="h4" component="h1" gutterBottom>
        Power 6/45 Number Analysis
      </Typography>

      <Typography variant="body1" color="text.secondary" gutterBottom>
        Analysis of {data.length} draws from Vietlott Power 6/45
      </Typography>
...
export default NumberAnalysis645; 